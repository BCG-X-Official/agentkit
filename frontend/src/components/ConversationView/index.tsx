...
import { generateUUID, MAX_TOTAL_MESSAGES_WITHOUT_TOKEN } from "~/utils"
...
const ConversationView = () => {
  ...
  const messageLimitReached =
    !localStorage.getItem("openaiApiKey") && settingsStore.setting.totalMessages >= MAX_TOTAL_MESSAGES_WITHOUT_TOKEN
  const disableMessageSend = lastMessage?.status === "LOADING" || messageLimitReached
  ...
  return (
    <div className="relative flex h-full max-h-full w-full flex-col items-start justify-start bg-neutral dark:bg-base-300">
      ...
      <div className="absolute bottom-0 flex w-full flex-row items-center justify-center gap-csm !bg-neutral px-8 py-csm xl:px-16 dark:!bg-base-200">
        ...
        <MessageTextarea disabled={disableMessageSend} sendMessage={sendMessageToCurrentConversation} />
