...
export const env = createEnv({
  ...
  server: {
    ...
    // For VERCEL deployment we want to use the rewrites() for API calls, thus we need to use /api/v1/ as the base URL (no `https`)
    NEXT_PUBLIC_API_URL: process.env.VERCEL ? z.string().min(1) : z.string().url(),
  ...
  client: {
    NEXT_PUBLIC_ENABLE_MESSAGE_FEEDBACK: z.enum(['true', 'false']).optional().transform((str) => str === 'true'),
    // For VERCEL deployment we want to use the rewrites() for API calls, thus we need to use /api/v1/ as the base URL (no `https`)
    NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_VERCEL_URL ? z.string().min(1) : z.string().url(),
    NEXT_PUBLIC_USE_AUTH: z.enum(['true', 'false']).default('false').transform((str) => str === 'true'),
