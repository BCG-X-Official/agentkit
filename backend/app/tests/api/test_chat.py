...
from app.schemas.streaming_schema import StreamingDataTypeEnum, StreamingSignalsEnum
...
def mock_llm_call(llm: BaseLanguageModel):  # pylint: disable=redefined-outer-name
    with patch(
        "app.services.chat_agent.tools.applications.project_scheduling.clarify_tool.clarify_tool.get_llm",
        return_value=llm,
    ):
        yield
