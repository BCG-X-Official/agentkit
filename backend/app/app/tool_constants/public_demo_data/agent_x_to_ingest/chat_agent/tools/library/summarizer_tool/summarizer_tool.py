...
class SummarizerTool(ExtendedBaseTool):
    ...
    def from_config(cls, config: ToolConfig, common_config: AgentAndToolsConfig, **kwargs):
        ...
        return cls(
            ...
            max_token_length=common_config.max_token_length,
            summarize_prompt_template=PromptTemplate(template=prompt_template_message, input_variables=["text"]),
        )
    ...
    async def _arun(self, query: str, run_manager: Optional[AsyncCallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            if get_token_length(query) <= self.max_token_length:
                ...
                chain = load_summarize_chain(self.llm, chain_type="stuff", prompt=self.summarize_prompt_template)
            else:
            ...
            response = await chain.arun(docs, callbacks=callbacks)
            return response
