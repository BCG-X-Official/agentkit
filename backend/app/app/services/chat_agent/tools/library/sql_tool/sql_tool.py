...
class SQLTool(ExtendedBaseTool):
    ...
    async def _aquery_with_schemas(
        ...
        logger.info(f"Filtered tables upper case: {filtered_tables_upper_case}")
        db_table_infos = sql_tool_db.db_info.tables if sql_tool_db and sql_tool_db.db_info is not None else []
        logger.info(f"DB table infos: {db_table_infos}")
        table_schemas = "\n".join(
            [
        ...
        logger.info(f"Table schemas1: {table_schemas}")
        question_messages = [
            SystemMessage(content=self.system_context),
            HumanMessage(
