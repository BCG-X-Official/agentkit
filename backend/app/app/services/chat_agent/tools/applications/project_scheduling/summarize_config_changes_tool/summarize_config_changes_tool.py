...
class ProjectSchedulingSummarizeConfigChangesTool(ExtendedBaseTool):
    ...
    async def _arun(self, query: str, run_manager: Optional[AsyncCallbackManagerForToolRun] = None) -> str:
        ...
        try:
            new_parameter_config_path = (
                Path(OPTIMIZATION_CONFIG_PATH) / f"optimization_config_{get_conversation_id(run_manager.tags)}_new.json"
            )
            old_parameter_config_path = (
                Path(OPTIMIZATION_CONFIG_PATH) / f"optimization_config_{get_conversation_id(run_manager.tags)}_old.json"
            )
            ...
            new_parameter_config = self.load_config_file(new_parameter_config_path)

            if os.path.isfile(old_parameter_config_path):
                current_parameter_config = self.load_config_file(old_parameter_config_path)
            else:
                current_parameter_config = self.load_config_file(
            ...
            messages = [
                ...
                HumanMessage(
                    content=self.prompt_message.format(
                        new_parameter_config=new_parameter_config,
                        current_parameter_config=current_parameter_config,
